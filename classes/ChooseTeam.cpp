...
ChooseTeam::ChooseTeam(Screen* screen, TwoVsTwo* parentState, bool playerType, float start) : SubState (screen, start, 0.5f, 9, 30) {
...
void ChooseTeam::keyInput(int key) {
    if (this->focused) {
        MenuElement* temp;
        switch (key) {
            case KEY_UP:
            ...
            case KEY_SPACE:
                switch (cursorPosition) {
                    case 0:
                        this->nextState = new NewTeam(this->screen, this->parentState, this->start);
                        break;
                    case 1:
                        this->nextState = new ChooseTeamSubState(this->screen, this->parentState, this->playerType, this->start);
                        break;
                    case 2:
...
void ChooseTeam::update(int deltaTime) {
...
void ChooseTeam::toggleFocus() {
    this->focused = !this->focused;
    this->menuElements[this->cursorPosition]->toggleActive();
}

bool ChooseTeam::getPlayerType() {
