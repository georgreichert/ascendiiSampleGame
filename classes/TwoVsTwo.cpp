...
TwoVsTwo::TwoVsTwo(Screen* screen) : GameState(screen) {
    std::string player[5] = {
        "####  ##      ####  ##  ## ###### #### ",
        "## ## ##     ##  ##  ####  ##     ## ##",
        "####  ##     ######   ##   ####   #### ",
    ...
    this->player1ChoosePlayer = new ChooseTeam(this->screen, this);
    ...
    this->player2ChoosePlayer = new ChooseTeam(this->screen, this, false, 0.5f);
...
void TwoVsTwo::keyInput(int key) {
...
void TwoVsTwo::update(int deltaTime) {
    int currentRow = 1;
    ...
    if (this->startFightAnim) {
        ...
        if (this->startFightAnimTimer < 1000) {
            ...
            this->nextState = new TeamFight(screen, this->team1, this->team2);
        }
    }
}

void TwoVsTwo::toggleActiveSubState() {
    this->activeSubState->toggleFocus();
...
void TwoVsTwo::startFight(Team* team, float start) {
    if (start == 0.0f) {
        this->team1 = team;
    } else {
        this->team2 = team;
    }
    if (this->team1 != nullptr && this->team2 != nullptr) {
        this->startFightAnim = true;
        this->startFightAnimTimer = 0;
    }
}

void TwoVsTwo::abortFight(float start) {
    if (start == 0.0f) {
        this->team1 = nullptr;
    } else {
        this->team2 = nullptr;
